...
### 7. Lociros (formerly Levla; Active, featured on the homepage above Grammario)

Live at [https://lociros.com](https://lociros.com). CEFR-calibrated graded readers for Japanese, Arabic, Italian, and Russian (A1–B2). An LLM (OpenRouter) drafts each passage under per-level grammar constraints; a morphological analyzer (Sudachi for Japanese, CAMeL Tools for Arabic, spaCy for Italian, razdel + pymorphy3 for Russian) validates the draft against rule-based CEFR constraints and forces a rewrite on failure. Tap any word for reading, grammar, gloss, Japanese kanji with KanjiVG stroke order, or the Arabic root and وزن. Rating a text too easy / just right / too hard adjusts placement and picks the next passage. Guest progress uses a device UUID; Supabase Auth accounts keep level, saved words, and history across devices. **Stack:** Next.js 16, FastAPI, SQLAlchemy, Supabase (Postgres + Auth), Sudachi, CAMeL Tools, spaCy, pymorphy3, OpenRouter. **Code:** [https://github.com/mserdukoff/lociros](https://github.com/mserdukoff/lociros). Local source at `~/Desktop/Programming/levla` (the local directory still uses the old name).
